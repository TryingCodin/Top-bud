window.addEventListener("DOMContentLoaded", () => {
  ...
  const headerMenuListFirst = document.querySelector(
    ".header__menu--list-dropdown-first"
  );
  const nextBtnFirst = document.querySelectorAll(".next-btn--first");
  const listDropdownFirst = document.querySelector(
    ".header__menu--list-dropdown-first"
  );
  const listDropdownFirstItems = listDropdownFirst.querySelectorAll(
    ":scope > .header__menu--list-dropdown-item"
  );
  ...
  nextBtnFirst.forEach((btn) => {
    btn.addEventListener("click", () => {
      listDropdownFirstItems.forEach((li) => {
        btn
          .closest(".header__menu--list-dropdown-item")
          .classList.add("list-items--z-ind");
  ...
  backBtnSecond.forEach((btn) => {
    btn.addEventListener("click", () => {
      btn
        .closest(".header__menu--list-dropdown-second")
        .classList.remove("list_show-second");
  ...
  backBtnThird.forEach((btn) => {
    btn.addEventListener("click", () => {
      btn
        .closest(".header__menu--list-dropdown-third")
        .classList.remove("list_show-third");

      btn
        .closest(".header__menu--list-dropdown-second")
        .classList.remove("list_hide-second");
    });
  });
  ...
  closeMenuBtn.addEventListener("click", () => {
    ...
    document
      .querySelectorAll(".header__menu--list-dropdown-second")
      .forEach((list) => {
        if (list.classList.contains("list_show-second"))
          list.classList.add("list_hide-second--non-visible");
    ...
    document
      .querySelectorAll(".header__menu--list-dropdown-third")
      .forEach((list) => {
        if (list.classList.contains("list_show-third"))
          list.classList.add("list_hide-third--non-visible");
  ...
  function removeUnnecessaryKeys(exceptKey) {
    for (let i = 0; i < sessionStorage.length; i++) {
      ...
      if (key !== exceptKey && key !== "sort-option")
  ...
  /*<----- Assortment ----->*/
  const assortmentDropdown = document.querySelectorAll(
    ".assortment__item--dropdown"
  );

  assortmentDropdown.forEach((item) => {
    item.addEventListener("click", () => {
      item
        .querySelector(".assortment__item--dropdown-list")
        .classList.toggle("list_open");
      item
        .querySelector(".assortment__item--dropdown-btn")
        .classList.toggle("btn_back");
    });
  });

  /*<----- Recall modal ----->*/
  const recallModalFields = document.querySelectorAll(
    '.recall__form input[type="text"], .recall__form textarea'
  );
